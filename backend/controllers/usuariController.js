...
const deleteUser = (req, res) => {
    ...
    // Llista de taules dependents per eliminar
    ...
    // Executar les eliminacions dependents en sèrie per evitar problemes
    ...
    deleteDependents.forEach((query, index) => {
        db.query(query, [id], (err) => {
            ...
            // Quan s'hagi executat l'última consulta, procedir a eliminar l'usuari
...
const updateProfile = (req, res) => {
    ...
    // Accedim directament als camps de req.body
    const nom = req.body.nom;
    const cognoms = req.body.cognoms;
    const correu = req.body.correu;
    const contrasenya = req.body.contrasenya;

    console.log('Valors obtinguts directament:', { nom, cognoms, correu, contrasenya });

    // Validem només que hi hagi contrasenya
    if (!contrasenya) {
    ...
    Usuari.getUserById(userId, (err, user) => {
        ...
        bcrypt.compare(contrasenya, user.contrasenya, (err, isMatch) => {
            ...
            // Si no es proporciona nom, cognoms o correu, mantenim els valors existents
